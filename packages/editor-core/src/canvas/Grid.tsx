...
export const Grid: React.FC<GridProps> = ({ viewport, settings }) => {
  ...
  const linesX = Math.ceil(viewport.width / gridSize) + 1;
  const linesY = Math.ceil(viewport.height / gridSize) + 1;
  ...
  const majorLinesX = Math.ceil(viewport.width / majorGridSize) + 1;
  const majorLinesY = Math.ceil(viewport.height / majorGridSize) + 1;
